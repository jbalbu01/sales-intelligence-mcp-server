...
import { z } from "zod";
import { gongPost, isGongConfigured } from "../services/gong-client.js";
...
Examples:
  - "Get the transcript for call 123456" -> call_id='123456'`,
    ...
    async (params) => {
      try {
        const data = await gongPost<{ callTranscripts: GongTranscript[] }>(
          "/calls/transcript",
        ...
        if (params.response_format === ResponseFormat.JSON) {
          return { content: [{ type: "text" as const, text: JSON.stringify({ callId: params.call_id, sentences }, null, 2) }] };
        }
...
Examples:
  ...
  - "What topics were discussed in call 789?" -> call_id='789'`,
    ...
    async (params) => {
      try {
        const data = await gongPost<{ calls: GongCallDetails[] }>(
          "/calls/extensive",
          {
            filter: { callIds: [params.call_id] },
            contentSelector: {
  ...
  - "Find calls with john@acme.com in January" -> email='john@acme.com', from_date='2024-01-01T00:00:00Z', to_date='2024-02-01T00:00:00Z'`,
    ...
    async (params) => {
      try {
        ...
        if (!calls.length) {
          return {
            content: [{ type: "text" as const, text: `No calls found with participant ${params.email} in the specified date range.` }],
          };
        }

        if (params.response_format === ResponseFormat.JSON) {
          return { content: [{ type: "text" as const, text: JSON.stringify({ total: calls.length, calls }, null, 2) }] };
        }
