...
function getHeaders() {
    ...
    if (!token) {
        throw new Error("LinkedIn credentials not configured. Set LINKEDIN_ACCESS_TOKEN environment variable. " +
...
async function linkedInGet(endpoint, params) {
...
async function linkedInPost(endpoint, data) {
    const response = await axios.post(`${LINKEDIN_API_BASE}${endpoint}`, data, {
        headers: getHeaders(),
        timeout: API_TIMEOUT,
    });
    return response.data;
...
// ─── Helpers ──────────────────────────────────────────────────
function formatProfileMarkdown(p) {
    return [
...
function formatCompanyMarkdown(c) {
    return [
        `### ${c.name}`,
...
export function registerLinkedInTools(server) {
    ...
    server.registerTool("linkedin_search_leads", {
  ...
  - "CTO at Acme Corp" -> title='CTO', company_name='Acme Corp'`,
        ...
        try {
            ...
            if (params.first_name)
                searchParams.firstName = params.first_name;
            if (params.last_name)
                searchParams.lastName = params.last_name;
            if (params.title)
                searchParams.title = params.title;
            if (params.company_name)
                searchParams.currentCompany = params.company_name;
            if (params.industry)
                searchParams.industry = params.industry;
            if (params.geography)
                searchParams.geoRegion = params.geography;
            if (params.seniority)
                searchParams.seniorityLevel = params.seniority;
            const data = await linkedInGet("/salesNavigatorProfiles", searchParams);
            const leads = data.elements || [];
            if (!leads.length) {
                return { content: [{ type: "text", text: "No leads found matching your criteria." }] };
            }
            if (params.response_format === ResponseFormat.JSON) {
                return { content: [{ type: "text", text: JSON.stringify({ total: data.paging?.total || leads.length, count: leads.length, offset: params.offset, leads }, null, 2) }] };
            }
            const lines = [
            ...
            return { content: [{ type: "text", text: handleApiError(error, "LinkedIn") }], isError: true };
        }
    });
  ...
  - "Get profile for linkedin.com/in/johndoe" -> linkedin_url='https://linkedin.com/in/johndoe'`,
        ...
        try {
            if (!params.linkedin_url && !params.member_id) {
                return { content: [{ type: "text", text: "Error: Provide either linkedin_url or member_id." }], isError: true };
            }
            const lookupParams = {};
            if (params.member_id) {
                ...
                if (match)
                    lookupParams.vanityName = match[1];
                else
                    lookupParams.profileUrl = params.linkedin_url;
            }
            const data = await linkedInGet("/salesNavigatorProfiles/lookup", lookupParams);
            ...
            return { content: [{ type: "text", text: `# LinkedIn Profile\n\n${formatProfileMarkdown(data)}` }] };
        }
        catch (error) {
            return { content: [{ type: "text", text: handleApiError(error, "LinkedIn") }], isError: true };
        }
    ...
    server.registerTool("linkedin_search_companies", {
  ...
  - "Find Acme Corp on LinkedIn" -> company_name='Acme Corp'`,
        ...
        try {
            ...
            if (params.keywords)
                searchParams.keywords = params.keywords;
            if (params.company_name)
                searchParams.companyName = params.company_name;
            if (params.industry)
                searchParams.industry = params.industry;
            if (params.min_employees)
                searchParams.staffCountMin = params.min_employees;
            if (params.max_employees)
                searchParams.staffCountMax = params.max_employees;
            if (params.geography)
                searchParams.geoRegion = params.geography;
            const data = await linkedInGet("/salesNavigatorCompanies", searchParams);
            const companies = data.elements || [];
            if (!companies.length) {
                return { content: [{ type: "text", text: "No companies found matching your criteria." }] };
            }
            if (params.response_format === ResponseFormat.JSON) {
                return { content: [{ type: "text", text: JSON.stringify({ total: data.paging?.total || companies.length, count: companies.length, offset: params.offset, companies }, null, 2) }] };
            }
            const lines = [
