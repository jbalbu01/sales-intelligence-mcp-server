...
const TriggerWebhookEnrichmentSchema = z.object({
    webhook_url: z.string().url().describe("The Clay webhook URL for the target enrichment table"),
    data: z.record(z.unknown()).describe("Key-value data to send to the webhook (e.g. {email: 'x@y.com', company: 'Acme'})"),
    response_format: z.nativeEnum(ResponseFormat).default(ResponseFormat.MARKDOWN)
        .describe("Output format: 'markdown' or 'json'"),
}).strict();
// ─── Helpers ──────────────────────────────────────────────────
function formatPersonMarkdown(p) {
    return [
...
// ─── Tool Registration ────────────────────────────────────────
export function registerClayTools(server) {
    // ── clay_enrich_person ──
    server.registerTool("clay_enrich_person", {
  ...
  - "Find info on Jane Doe at acme.com" -> first_name='Jane', last_name='Doe', company_domain='acme.com'`,
        ...
        try {
            if (!params.email && !params.linkedin_url && !(params.first_name && params.last_name)) {
                return {
                    content: [{ type: "text", text: "Error: Provide at least email, linkedin_url, or first_name + last_name." }],
                    isError: true,
                };
            }
            const body = {};
            ...
            if (params.company_domain)
                body.companyDomain = params.company_domain;
            const data = await clayPost("/people/enrich", body);
            if (!data || (!data.fullName && !data.email)) {
                return { content: [{ type: "text", text: "No enrichment data found for this person." }] };
            }
            if (params.response_format === ResponseFormat.JSON) {
                return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
            }
            return { content: [{ type: "text", text: `# Person Enrichment\n\n${formatPersonMarkdown(data)}` }] };
        }
        catch (error) {
            return { content: [{ type: "text", text: handleApiError(error, "Clay") }], isError: true };
        }
    });
    // ── clay_enrich_company ──
    server.registerTool("clay_enrich_company", {
  ...
  - "Get info on Acme Corp" -> company_name='Acme Corp'`,
        ...
        try {
            if (!params.domain && !params.company_name) {
                return {
                    content: [{ type: "text", text: "Error: Provide either domain or company_name." }],
                    isError: true,
                };
            }
            const body = {};
            ...
            const data = await clayPost("/companies/enrich", body);
            if (!data || (!data.name && !data.domain)) {
                return { content: [{ type: "text", text: "No enrichment data found for this company." }] };
            }
            if (params.response_format === ResponseFormat.JSON) {
                return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
            }
            return { content: [{ type: "text", text: `# Company Enrichment\n\n${formatCompanyMarkdown(data)}` }] };
        }
        catch (error) {
            return { content: [{ type: "text", text: handleApiError(error, "Clay") }], isError: true };
        }
    ...
    server.registerTool("clay_trigger_enrichment", {
...
Examples:
  - Trigger enrichment: webhook_url='https://...', data={"email":"john@acme.com","name":"John Doe"}`,
        ...
        try {
            const result = await clayWebhookPost(params.webhook_url, params.data);
            if (params.response_format === ResponseFormat.JSON) {
                return { content: [{ type: "text", text: JSON.stringify({ status: "submitted", webhook: params.webhook_url, data: params.data, response: result }, null, 2) }] };
            }
            return {
                content: [{
                        type: "text",
...
//# sourceMappingURL=clay-tools.js.map