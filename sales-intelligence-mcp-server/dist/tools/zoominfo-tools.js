...
const SearchContactSchema = z.object({
    ...
    department: z.string().optional()
        .describe("Department filter (e.g. 'Sales', 'Engineering', 'Marketing')"),
...
function formatContactMarkdown(c) {
    return [
...
// ─── Tool Registration ────────────────────────────────────────
export function registerZoomInfoTools(server) {
    // ── zoominfo_search_company ──
    server.registerTool("zoominfo_search_company", {
...
Examples:
  ...
  - "SaaS companies with 500+ employees" -> industry='Technology', min_employees=500`,
        ...
        try {
            ...
            if (params.domain)
                body.websiteURL = params.domain;
            if (params.industry)
                body.industry = params.industry;
            if (params.min_employees)
                body.employeeCountMin = params.min_employees;
            if (params.max_employees)
                body.employeeCountMax = params.max_employees;
            if (params.min_revenue)
                body.revenueMin = params.min_revenue;
            if (params.country)
                body.country = params.country;
            const data = await zoomInfoPost("/search/company", body);
            ...
            return { content: [{ type: "text", text: handleApiError(error, "ZoomInfo") }], isError: true };
        }
    });
    // ── zoominfo_search_contact ──
    server.registerTool("zoominfo_search_contact", {
...
Examples:
  ...
  - "C-level contacts in engineering" -> management_level='c-level', department='Engineering'`,
        ...
        try {
            ...
            if (params.email)
                body.emailAddress = params.email;
            if (params.job_title)
                body.jobTitle = params.job_title;
            if (params.management_level)
                body.managementLevel = params.management_level;
            if (params.department)
                body.department = params.department;
            if (params.company_name)
                body.companyName = params.company_name;
            if (params.company_domain)
                body.websiteURL = params.company_domain;
            const data = await zoomInfoPost("/search/contact", body);
            const contacts = data.data || [];
            if (!contacts.length) {
                return { content: [{ type: "text", text: "No contacts found matching your criteria." }] };
            }
            if (params.response_format === ResponseFormat.JSON) {
                return { content: [{ type: "text", text: JSON.stringify({ total: data.maxResults, count: contacts.length, page: params.page, contacts }, null, 2) }] };
            }
            const lines = [
    ...
    server.registerTool("zoominfo_get_org_chart", {
...
Examples:
  ...
  - "Engineering leadership at company 12345" -> company_id=12345, department='Engineering'`,
        ...
        try {
            const body = { companyId: params.company_id };
            if (params.department)
                body.department = params.department;
            const data = await zoomInfoPost("/lookup/orgchart", body);
            const entries = data.data || [];
            if (!entries.length) {
                return { content: [{ type: "text", text: `No org chart data found for company ${params.company_id}.` }] };
            }
            if (params.response_format === ResponseFormat.JSON) {
                return { content: [{ type: "text", text: JSON.stringify({ companyId: params.company_id, entries }, null, 2) }] };
            }
            const lines = [
                `# Org Chart — Company ${params.company_id}`,
                `${entries.length} people found.\n`,
            ];
    ...
    server.registerTool("zoominfo_get_tech_stack", {
  ...
  - "Tech stack for company 12345" -> company_id=12345`,
        ...
        try {
            if (!params.company_id && !params.domain) {
                return { content: [{ type: "text", text: "Error: Provide either company_id or domain." }], isError: true };
            }
            const body = {};
            if (params.company_id)
                body.companyId = params.company_id;
            if (params.domain)
                body.websiteURL = params.domain;
            const data = await zoomInfoPost("/lookup/technology", body);
            const techs = data.data || [];
            if (!techs.length) {
                return { content: [{ type: "text", text: "No tech stack data available for this company." }] };
            }
            if (params.response_format === ResponseFormat.JSON) {
                return { content: [{ type: "text", text: JSON.stringify({ technologies: techs }, null, 2) }] };
            }
            // Group by category
            const grouped = {};
...
//# sourceMappingURL=zoominfo-tools.js.map