...
export async function zoomInfoPost<T>(endpoint: string, data?: Record<string, unknown>): Promise<T> {
  const response = await getClient().post<T>(endpoint, data);
  return response.data;
}

export async function zoomInfoGet<T>(endpoint: string, params?: Record<string, unknown>): Promise<T> {
