...
import type { GongCall, GongCallDetails, GongTranscript, GongTranscriptSentence } from "../types.js";

// ─── Schemas ──────────────────────────────────────────────────

const SearchCallsSchema = z
  .object({
...
const GetCallTranscriptSchema = z
  .object({
...
const SearchCallsByParticipantSchema = z
  .object({
...
function formatCallMarkdown(call: GongCall): string {
...
export function registerGongTools(server: McpServer): void {
  // ── gong_search_calls ──
  server.registerTool(
    "gong_search_calls",
    {
  ...
  - "Get recent demo calls" -> search calls then filter by title containing 'demo'`,
    ...
    async (params) => {
      try {
        ...
        if (!calls.length) {
          return { content: [{ type: "text" as const, text: "No calls found in the specified date range." }] };
        }
        ...
        return { content: [{ type: "text" as const, text: handleApiError(error, "Gong") }], isError: true };
      }
    },
  );
...
Examples:
  - "Get the transcript for call 123456" -> call_id='123456'`,
    ...
    async (params) => {
      try {
        const data = await gongPost<{ callTranscripts: GongTranscript[] }>("/calls/transcript", {
          filter: { callIds: [params.call_id] },
        });

        const transcripts = data.callTranscripts || [];
        if (!transcripts.length || !transcripts[0].transcript?.length) {
          return { content: [{ type: "text" as const, text: `No transcript available for call ${params.call_id}.` }] };
        }

        const sentences: GongTranscriptSentence[] = transcripts[0].transcript;

        if (params.response_format === ResponseFormat.JSON) {
          return {
            content: [{ type: "text" as const, text: JSON.stringify({ callId: params.call_id, sentences }, null, 2) }],
          };
        }
...
Examples:
  ...
  - "What topics were discussed in call 789?" -> call_id='789'`,
    ...
    async (params) => {
      try {
        const data = await gongPost<{ calls: GongCallDetails[] }>("/calls/extensive", {
          filter: { callIds: [params.call_id] },
          contentSelector: {
            exposedFields: {
        ...
        const calls = data.calls || [];
        if (!calls.length) {
          return { content: [{ type: "text" as const, text: `No details found for call ${params.call_id}.` }] };
        }

        const call = calls[0];

        if (params.response_format === ResponseFormat.JSON) {
          return { content: [{ type: "text" as const, text: JSON.stringify(call, null, 2) }] };
        }
        ...
        if (topics.length) {
          ...
          for (const t of topics) lines.push(`- **${t.name}** (${formatDuration(t.duration)})`);
          lines.push("");
        }
        ...
        if (trackers.length) {
          ...
          for (const t of trackers) lines.push(`- **${t.name}**: ${t.count} occurrences`);
          lines.push("");
        }
        ...
        if (actionItems.length) {
          ...
          for (const a of actionItems) lines.push(`- ${a.snippet} *(Speaker ${a.speakerId})*`);
          lines.push("");
        }
        ...
        if (stats.length) {
          ...
          for (const s of stats) lines.push(`- **${s.name}**: ${s.value}`);
          lines.push("");
        }
  ...
  - "Find calls with john@acme.com in January" -> email='john@acme.com', from_date='2024-01-01T00:00:00Z', to_date='2024-02-01T00:00:00Z'`,
    ...
    async (params) => {
      try {
        ...
        if (!calls.length) {
          return {
            content: [
              {
                type: "text" as const,
                text: `No calls found with participant ${params.email} in the specified date range.`,
              },
            ],
          };
        }
        ...
        if (params.response_format === ResponseFormat.JSON) {
          return {
            content: [{ type: "text" as const, text: JSON.stringify({ total: calls.length, calls }, null, 2) }],
          };
        }
